@forelse ($subscriptions as $subscription)
    ...
    <div class="overflow-hidden rounded-xl bg-white shadow-lg ring-1 ring-gray-900/5">
        <div class="flex flex-wrap items-center justify-between gap-4 border-b border-gray-200 bg-gray-50 p-4 sm:px-6">
            ...
            <span class="inline-flex items-center gap-x-1.5 rounded-full bg-{{ $statusConfig['color'] }}-100 px-3 py-1 text-sm font-semibold text-{{ $statusConfig['color'] }}-800">
                ...
                @switch($status)
                        ...
                        <div class="rounded-md bg-blue-50 p-4">
                            <div class="flex">
                                ...
                                <div class="ml-3 flex-1">
                                    <p class="text-sm text-blue-700">Pesanan Anda sedang menunggu konfirmasi pembayaran dari Admin.</p>
                                </div>
                            </div>
                        </div>
                        @break
                    @case('dijeda')
                        <div class="rounded-md bg-yellow-50 p-4 text-sm text-yellow-800">
                            <h3 class="font-semibold">Langganan Dijeda</h3>
                            <p class="mt-1">
                                @if ($subscription->latestPauseHistory)
                                    Pengiriman dihentikan dari
                                ...
                                @else
                                    Pengiriman makanan Anda sedang dihentikan sementara.
                                @endif
                            </p>
                        </div>
                        @break
                    @case('dibatalkan')
                        <div class="rounded-md bg-red-50 p-4 text-sm text-red-700">
                            <p>Langganan ini telah dibatalkan.</p>
                        </div>
                        @break
                @endswitch

                @if ($status !== 'pending')
                    <div class="space-y-4 {{ in_array($status, ['dijeda', 'dibatalkan']) ? 'border-t border-gray-200 pt-4' : '' }}">
                        <dl class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-1 sm:gap-2">
