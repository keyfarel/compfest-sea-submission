    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            ...
            function updateButtons() {
                // Sembunyikan tombol 'prev' jika scroll sudah di paling kiri (awal)
                // Kondisi: currentScroll lebih kecil dari 1 (yaitu 0)
                prevBtn.classList.toggle('hidden', currentScroll < 1);

                // Sembunyikan tombol 'next' jika scroll sudah di paling kanan (akhir)
                // Kondisi: currentScroll sudah mencapai atau melebihi batas scroll maksimum
                nextBtn.classList.toggle('hidden', currentScroll >= totalScrollableWidth - 1);
            }

            function handlePrevClick() {
                // Tidak perlu lagi memeriksa 'disabled' karena tombolnya akan hilang
            ...
            function initializeCarousel() {
                ...
                if (items.length > visibleItems) {
                    // Tombol 'next' selalu muncul di awal, 'prev' akan di-handle oleh updateButtons
